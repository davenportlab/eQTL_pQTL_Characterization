...
for (qtl.id in qtl.ids) {
    ...
    for (pc in 1:5) {
        ...
        ME.pc = paste0(ME, "_", pc)

        results <- rbindlist(mclapply(qtl.snp.set, function(snp) {

            variant.design <- data.mtx[,c(ME.pc, snp, covs, "GAinS.ID")]

            f.null <- as.formula(paste0(ME.pc, "~", paste0(covs, collapse="+"), "+(1|GAinS.ID)"))
            model.null <- lmer(f.null, data=variant.design, REML=FALSE)

            f.alt <- as.formula(paste0(ME.pc, "~`", snp , "`+", paste0(covs, collapse="+"), "+(1|GAinS.ID)"))
        ...
        write.table(results, paste0(ME.pc, "-", qtl.id, ".tsv"), quote=F, row.names=F, col.names=F, sep="\t")
    }
}
