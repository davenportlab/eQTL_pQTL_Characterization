...
process IDENTIFY_TSS_REGIONS {
    ...
    script:
        ...
        awk -F '\t' 'OFS="\t" {
            print \$4 "-Left", \$1, \$2, \$3, \$6;
            }' tss.100.left.flank.filtered.bed > tss.100.left.flank.saf
        
        awk -F '\t' 'OFS="\t" {
            print \$4 "-Right", \$1, \$2, \$3, \$6;
            }' tss.100.right.flank.filtered.bed > tss.100.right.flank.saf
        
        echo -e "GeneID\tChr\tStart\tEnd\tStrand" > tss.all.saf
        cat tss.100.saf tss.100.left.flank.saf tss.100.right.flank.saf >> tss.all.saf
        """
}
...
process TSS_ENRICHMENT_SCORES {
    ...
    script:
        ...
        TSS=\$(awk '{ if (\$1 ~ "TSS") { s += \$7; } } END { print s; }' tss_counts.tsv)
        LEFT=\$(awk '{ if (\$1 ~ "Left") { s += \$7; } } END { print s; }' tss_counts.tsv)
        RIGHT=\$(awk '{ if (\$1 ~ "Right") { s += \$7; } } END { print s; }' tss_counts.tsv)
...
workflow {
    ...
    TSS_ENRICHMENT_SCORES(
        samples_list,
        IDENTIFY_TSS_REGIONS.out.tss_saf
    )
