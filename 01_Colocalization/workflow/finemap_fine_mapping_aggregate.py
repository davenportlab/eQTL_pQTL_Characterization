...
cred_output_df.to_csv(f'chr{chr_input}_credible_sets.tsv', sep='\t', index=False)
...
with open(f'chr{chr_input}_pips.tsv', 'w') as f_out:
...
for file_name in sorted(os.listdir('snps/')):
    ...
    snps.to_csv(f'chr{chr_input}_pips.tsv', sep='\t', header=False, index=False, mode='a')
