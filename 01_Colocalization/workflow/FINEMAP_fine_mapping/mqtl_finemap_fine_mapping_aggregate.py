...
for file_name in os.listdir('credible_sets/'):
    ...
    module_qtl_name = re.sub('\\.cred.*', '', file_name)
    ...
    for signal in range(k):
        ...
        cred_output['Module_QTL'] += [module_qtl_name] * n_snps
...
cred_output_df = pd.DataFrame(cred_output).sort_values(
    by=['Module_QTL', 'Post_Prob_k', 'Credibility_Set', 'Prob_SNP_in_CS'], 
    ascending=[True, False, True, False]
)

cred_output_df.to_csv(f'module_qtl_credible_sets.tsv', sep='\t', index=False)
...
with open(f'module_qtl_pips.tsv', 'w') as f_out:
    header = snps.columns.tolist()
    header = ['module_qtl'] + header + ['\n']
...
for file_name in sorted(os.listdir('snps/')):
    ...
    module_qtl_name = re.sub('\\.snp', '', file_name)

    snps = pd.read_csv(file_path, sep=' ', index_col=0).sort_values(by='position')
    snps.insert(0, 'Module', [module_qtl_name] * snps.shape[0])

    snps.to_csv(f'module_qtl_pips.tsv', sep='\t', header=False, index=False, mode='a')
